...
def _send_letter_email(letter, status, note=''):
    ...
    try:
        send_mail(
            ...
            from_email=getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@smartwritingfinance.com'),
...
def admin_dashboard(request):
    ...
    stats = {
        ...
        'total_paid_out':   letters.filter(status='approved').aggregate(t=Sum('earnings'))['t'] or 0,
...
def admin_letters(request):
    ...
    if search:
        qs = qs.filter(Q(title__icontains=search) | Q(user__username__icontains=search) | Q(user__email__icontains=search))
    ...
    return render(request, 'adminpanel/letters.html', {
        'letters': letters, 'status': status, 'search': search, 'counts': counts,
    })
...
def admin_letter_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve' and letter.status == 'pending':
            ...
            messages.success(request, f"Letter approved. ${payment} credited to {letter.user.username}. Email sent.")
            ...
            messages.warning(request, f"Letter declined. Email sent to {letter.user.username}.")
...
def admin_user_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == 'toggle_active':
            ...
            send_mail(subject, body,
                      getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@smartwritingfinance.com'),
    ...
    return render(request, 'adminpanel/user_detail.html', {
        'u': u, 'letters': letters, 'deposits': deposits, 'withdrawals': withdrawals,
    })
...
def admin_deposits(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'confirm' and dep.status == 'pending':
            ...
            send_mail(
                ...
                f"— Smart Writing Finance Team",
                getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@smartwritingfinance.com'),
                [dep.user.email], fail_silently=True,
            )
    ...
    return render(request, 'adminpanel/deposits.html', {
        'deposits': deposits, 'status': status, 'counts': counts,
    })
...
def admin_withdrawals(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'complete' and w.status in ('pending', 'processing'):
            ...
            send_mail(
                ...
                f"— Smart Writing Finance Team",
                getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@smartwritingfinance.com'),
                [w.user.email], fail_silently=True,
            )
            ...
            messages.warning(request, "Withdrawal rejected. Balance refunded. Email sent.")
    ...
    return render(request, 'adminpanel/withdrawals.html', {
        'withdrawals': withdrawals, 'status': status, 'counts': counts,
    })
